...
function Home() {
    ...
    const getDataAllBanner = async () => {
        ...
        const fetchData: HttpResponseGetAll<XModel> = await makeHttpCall<HttpResponseGetAll<XModel>>(requestDataAll);


        setBanners(fetchData.data.map((row: XModel) => {
    ...
    return (
        <>
            {reset && <SwipeableTextMobileStepper baners={banners} />}

import {
    Card, CardContent, MenuItem, Select, TextField
} from "@mui/material";

import { GridColDef, GridRowsProp, useGridApiContext } from "@mui/x-data-grid";
import FullFeaturedCrudGrid from "./AuditTable";
import { TabType, TableType } from "./MeasureBak";

const discountOptions = ['Pareto', 'P Chart', 'Process Audit', 'Startification', 'Brainstorming'
, 'ISO Plot', 'AAA', 'Process Mapping', 'Concentration Chart'];

function CustomEditComponent(props: {id: string, value: string, field: string}) {
...
function Analyze() {
    ...
    const initialRows: GridRowsProp = JSON.parse(initialRowsString !== null ? initialRowsString :
        JSON.stringify([]));
        ...
        <>
            <Card className="outerContainer">
                <CardContent style={{ paddingBottom: 0 }}>
                    <FullFeaturedCrudGrid hasAttachment tableTitle='Suspected Sources Of Variation' initialColumns={columnsDetails} initialRows={initialRows} data={TableType.Audit} tab={TabType.Process}></FullFeaturedCrudGrid>
                    {/* <Grid container>

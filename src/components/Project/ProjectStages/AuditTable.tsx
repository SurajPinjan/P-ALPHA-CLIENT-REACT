...
export default function FullFeaturedCrudGrid(props: { updateHandler?: (editData: GridValidRowModel) => void, saveHandler?: (newData: GridValidRowModel) => void, isCompare?: boolean, hasAttachment: boolean, initialColumns: GridColDef[], initialRows: GridRowsProp, data: TableType, tab: TabType, tableTitle: string, buttonTitle?: string, isClicked?: String, onSelect?: (subData: GridRowsProp) => void }) {
  ...
  React.useEffect(() => {
    if (props.isClicked && (props.isClicked != null) && (props.isClicked != 'null') && !isAudit) {
  ...
  React.useEffect(() => {
    let entityFound: GridValidRowModel | undefined = findById(updateId);
  ...
  const handleDetailsClick = (subData: GridRowsProp) => {
    if (props.onSelect) { props.onSelect(subData) };
    //    setRows(rows.filter((row) => row.id !== id));
  };
  ...
  const columnsDetails: GridColDef[] = hasAttachment ? [...props.initialColumns, {
    field: 'url',
    headerName: 'Url',
    type: 'actions',
    width: 100,
  ...
  return (
    <>
      {imgRw && <ImagePreview url={imgRw.url} isOpen={isOpen} compare={isCompare} onClose={onClose}></ImagePreview>}
      {imgRw && <FileUpload urlExisting={imgRw.url} isOpen={isOpenUpload} onClose={onClose} onUpload={(urlSaved: string | undefined) => {
        if (imgRw) {
          imgRw.url = urlSaved;
        }
      }}></FileUpload>}
      ...
      >
        <DataGrid sx={{ border: 'none', padding: '15px' }}
          rows={rows}

...
import RolePermissionDialogue from "../../commons/Dialogues/RolePermmissionsDialogue";
import {
  RoleModel,
  RoleView,
  getModelFromViewRole,
  getViewFromModelRole,
} from "../../models/Role";
...
const RoleGrid: React.FC<RoleProps> = (props) => {
  // constants
  ...
  const [buttonTitle] = React.useState("Add Role");
  const [tableTitle] = React.useState("Role");
        ...
        <GridActionsCellItem
          ...
          onClick={() => {
            ...
            setTimeout(() => {
              ...
              setImgRw(dat);
              if (dat && dat.uid && dat.uid > 0) {
                setIsOpen(true);
              } else {
                alert("Role Must Be Saved Before Setting Permissions");
              }
            }, 200);
          }}
        />,
  ...
  const updateData = useCallback(
    async (viewData: RoleView) => {
      const requestDataCreate: HttpRequestData<
        HttpUpdateOneRequestBody<RoleModel>
      > = {
        entityName: ENTITY_NAME.ROLE,
        method: HTTP_METHOD.POST,
        operation: OPERATION.UPDATE_ONE,
        body: {
          data: getModelFromViewRole(viewData),
        },
      };

      const updatedData: HttpResponseUpdateOne<RoleModel> = await makeHttpCall<
        HttpResponseUpdateOne<RoleModel>,
        HttpUpdateOneRequestBody<RoleModel>
  ...
  const createData = useCallback(
    async (viewData: RoleView) => {
      const requestDataCreate: HttpRequestData<
        HttpCreateOneRequestBody<RoleModel>
      > = {
        entityName: ENTITY_NAME.ROLE,
        method: HTTP_METHOD.POST,
      ...
      const createdData: HttpResponseCreateOne<RoleModel> = await makeHttpCall<
        HttpResponseCreateOne<RoleModel>,
        HttpCreateOneRequestBody<RoleModel>
      >(requestDataCreate, store, navigate);
      setSavedId(-1);
  ...
  React.useEffect(() => {
    ...
    if (
      ...
      createData({
        role_name: entityFound.role_name,
        isDeleted: false,
